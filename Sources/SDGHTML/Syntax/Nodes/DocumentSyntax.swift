...
public struct DocumentSyntax : Syntax {
    ...
    public static func parse(source: String) -> Result<DocumentSyntax, SyntaxError> {
        return ListSyntax<ContentElementSyntax>.parse(source: source).map { content in
    ...
    /// - Parameters:
    ///     - content: The content.
    public init(content: ListSyntax<ContentElementSyntax>) {
    ...
    public var content: ListSyntax<ContentElementSyntax> {
        get {
            return _storage.children[DocumentSyntax.indices[.content]!] as! ListSyntax<ContentElementSyntax>
        }
        set {
