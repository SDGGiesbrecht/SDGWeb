...
internal func localize<L>(_ string: inout StrictString, for localization: L)
where L: InputLocalization {
  let opaqueLocalizationIndex = L.allCases.firstIndex(where: { $0.code == localization.code })!
  let localizationIndex = L.allCases.distance(
    from: L.allCases.startIndex,
    to: opaqueLocalizationIndex
  )
